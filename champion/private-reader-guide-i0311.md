# Private Reader Guide - I-0311

Updated: 2026-05-27

## Read First

Open the current private proof:

`rendered/final_private_i0309/Next-Token-final-private-publishable-surface-i0309.pdf`

This is the clean repaired proof, not the older reader-polish proof. It has 662 pages, 330 image objects, 5818 drawing/vector objects, 0 visible local-path residue hits, and 0 draft/process phrase hits. SHA-256: `3bafb20e3a5e51eb013c693afc606fd1b25ee4e3f4f4392470940367347a5f1f`.

## Use These Alongside It

- PDF proof pointer: `champion/final-private-pdf-pointer-i0309.md`
- Page-by-page PDF QA: `data/publishable_surface_page_qa_i0309.tsv`
- Publishable-surface QA summary: `data/publishable_surface_qa_i0309.tsv`
- Visual inventory: `data/final_private_visual_inventory_i0310.tsv`
- Visual page map: `data/final_private_visual_page_map_i0310.tsv`
- Visual page distribution: `data/final_private_visual_page_distribution_i0310.tsv`
- Visual map pointer: `champion/final-private-visual-page-map-pointer-i0310.md`
- Contact sheet: `rendered/final_inventory_i0302/Next-Token-final-private-visual-contact-sheet-i0302.pdf`
- Claim audit report: `champion/final-source-claim-quarantine-i0303.md`
- Champion package index: `champion/README.md`

## What The Current Proof Has Passed

- PDF surface QA: 11 pass / 0 fail
- Visual page-map QA: 12 pass / 0 fail
- Visual inventory mapping: 530 / 530 across 308 distinct visual-bearing pages
- Claim ledger: 327 supported / 0 needs-verification
- Book invariants: 103,526 words and 24 main chapters

## Still Not Final

Do not treat this as the done-enough edition yet. The next open gates are:

- I-0312: front matter, back matter, package labels, and PDF metadata polish
- I-0313: hostile visual sample audit for overlap, beauty, and professional page feel
- I-0314: final reader-continuity pass across opening, middle visual sections, and ending
- I-0315: requirement-by-requirement done-enough audit against `GOAL.md`

Private-use note: found/company/source-surface visuals remain for private reading and audit. This guide does not claim public distribution rights.
