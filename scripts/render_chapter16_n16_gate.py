...
RENDER_DIR = ROOT / "rendered" / "chapter16_i0083"
DATA_OUT = ROOT / "data" / "chapter16_n16_1_live_pdf_render_qa_i0083.tsv"
MANUSCRIPT_NOTE = ROOT / "manuscript" / "16-n16-1-live-pdf-render-qa.md"
CHAMPION_NOTE = ROOT / "champion" / "16-n16-1-live-pdf-render-qa.md"
...
def render_variant(title: str, blocks: list[str], variant: str) -> RenderResult:
    ...
    pdf_path = RENDER_DIR / f"chapter16_i0083_{variant}.pdf"
    text_path = RENDER_DIR / f"chapter16_i0083_{variant}.txt"
    doc.save(pdf_path)
    doc.close()
    ...
    for i, page in enumerate(rendered, start=1):
        ...
        image_path = RENDER_DIR / f"chapter16_i0083_{variant}_p{i:02d}.png"
        pix.save(image_path)
...
def qa_rows(parent: RenderResult, candidate: RenderResult) -> list[dict[str, str]]:
    ...
    rows = [
        ...
        },
        {
            ...
            "notes": "Visual inspection target is rendered/chapter16_i0083/chapter16_i0083_candidate_p02.png in the current layout.",
        },
        {
            "qa_id": "CH16PDF-003",
            "gate_id": "CH16PG-003",
        ...
        },
        {
            "qa_id": "CH16PDF-009",
            "gate_id": "CH16PG-009",
            "evidence": "live manuscript keeps only the LBNL/DOE paragraph under the N16-1 anchor; candidate render adds the full CH16Q-003 through CH16Q-006 mapping note without adding new quantities.",
            "result": "pass" if "energy-per-token model" in cand_text and "CH16Q-018" in cand_text and all(token in cand_text for token in ["CH16Q-003", "CH16Q-004", "CH16Q-005", "CH16Q-006"]) else "fail",
            "decision": "candidate_pdf_gate_passed",
            "notes": "Live prose changed only by replacing the LBNL/DOE CH16Q-003 through CH16Q-006 inline clusters with N16-1; no new quantitative claim row is introduced.",
        },
        {
            "qa_id": "CH16PDF-010",
            "gate_id": "CH16PG-010",
            ...
            "result": "pass" if ("CH16Q-003" in parent_text or "N16-1" in parent_text) and "N16-1" in cand_text and "CH16Q-003" in cand_text else "fail",
            "decision": "candidate_pdf_gate_passed",
    ...
    rows.extend(
        {
            ...
            "result": row["result"],
            "decision": "candidate_pdf_gate_passed" if row["result"] == "pass" else "do_not_merge_live",
            "notes": row["notes"],
        }
        for row in smoke_rows(
            ...
            image_specs=[
                ImageSmokeSpec(candidate.pages[(candidate.anchor_page or 1) - 1], "ANCHOR", min_unique_colors=8, min_colorfulness=0.5),
                ImageSmokeSpec(candidate.pages[-1], "FIGURE", min_unique_colors=8, min_colorfulness=0.0),
            ],
            artifacts=[parent.pdf, parent.text, *parent.pages, candidate.pdf, candidate.text, *candidate.pages],
        )
    )
    return rows
...
def write_notes(rows: list[dict[str, str]]) -> None:
    ...
    text = (
        ...
        "Pass I-0083 reruns the true local PDF/page-image proof after the N16-1 note anchor is integrated into the live LBNL/DOE paragraph. "
        "The generated PDFs, extracted text files, and 2x PNG page images live under `rendered/chapter16_i0083/` and remain untracked because rendered PDFs/PNGs are intentionally ignored.\n\n"
        f"Gate summary: {passed} pass or pass-with-caveat rows, {failed} fail rows in `data/chapter16_n16_1_live_pdf_render_qa_i0083.tsv`.\n\n"
        "Decision: N16-1 passes the production-text PDF gate for the current Chapter 16 opening, including same-page placement, text extraction, row-mapping integrity, inline CH16Q-017 and CH16Q-018 blocker visibility, operator-signal stability, no-new-quant control, and parent/candidate reversibility. "
        "The live prose change remains narrow: only CH16Q-003 through CH16Q-006 moved from inline LBNL/DOE cue clusters into N16-1.\n"
    )
    MANUSCRIPT_NOTE.write_text(text, encoding="utf-8")
    CHAMPION_NOTE.write_text("# Champion Chapter 16 N16-1 Live PDF Render QA\n\n" + text.split("\n\n", 1)[1], encoding="utf-8")
