...
Then GPT-4o pushed the interface in a different direction. OpenAI introduced GPT-4o in May 2024 as a model extending ChatGPT toward more natural multimodal interaction, with text and image capabilities rolling out and audio/video ambitions at the center of the announcement. [S-0046] this carefully. The book is about LLMs, not a general history of image or video models. But GPT-4o belongs here because it shows the chat interface stretching beyond typed text while keeping the assistant as the product frame.
...
The same rule applies to pricing. The Gemini Developer API pricing snapshot is already in the source ledger and should be used later for economics with care. [S-0061] Pricing pages mix free tiers, paid tiers, batch discounts, grounding, audio, thinking-token costs, and other billing semantics. They are not simple "cost per intelligence" tables. A future economics chapter preserves those distinctions rather than flattening Gemini into a single price point.
...
This chapter does not end with Google declared ahead or behind. That would be leaderboard theater. Its job is to put Google back into the race as a structurally different competitor.
...
## Llama 4: The Open-Weight Answer to the Reasoning Era
...
## Qwen 3.5, 3.6, and the Alibaba Acceleration
...
## Colossus: Speed As Infrastructure
...
## The Rental Market and the Price of Scarcity
...
## DGX Spark: The Supercomputer on a Desk
...
## Ryzen AI Max+ 395: A Hundred Gigabytes on a Laptop
...
The AI-factory framing was effective precisely because it collapsed a complex question into a single phrase. What kind of infrastructure does a world of AI inference need? NVIDIA answered: the same kind of infrastructure that powered the industrial revolution, but this time for tokens instead of physical goods. The metaphor was not literally true, but it was useful. It made compute procurement feel like an industrial necessity rather than a speculative technology bet. It made the datacenter feel like a factory floor rather than a back-office cost center. And it reframed the question from whether AI was overhyped to whether companies could afford not to have an AI factory of their own.

The Vera Rubin platform, launched at CES in January 2026 and spotlighted again at GTC, was the centerpiece of NVIDIA's next-generation argument. Jensen Huang described Vera Rubin NVL72 as the first rack-scale platform with Confidential Computing across CPU, GPU, and NVLink domains, protecting proprietary models during both training and inference. The roadmap also extended beyond Rubin to Rubin Ultra in 2027 and Feynman, announced as a future architecture, making the GTC presentation less a product launch and more a declaration that NVIDIA's hardware cadence now spanned the rest of the decade. [S-0183]

The competitive landscape around GTC made the AI-factory framing more urgent than it would have been in a monopoly market. AMD's MI350X, announced at the company's Advancing AI event in June 2025, matched the B200 on FP8 compute at 4,600 TFLOPS and exceeded it on memory with 288GB of HBM3E. AMD also previewed the MI400 series and the Helios rack-scale reference design, signaling that the rack-as-compute-unit concept was not NVIDIA's exclusive territory. Google's TPU v6, Amazon's Trainium3, and Microsoft's Maia accelerators all represented hyperscaler bets on custom silicon, while Huawei's Ascend series demonstrated that export controls could slow but not stop Chinese accelerator development. [S-0184] [S-0185]
...
DOE-SEAB recommended facility-level solutions including advanced cooling technologies, power and water reduction, waste-heat use, and experiments with onsite or facility-level electricity supply. [S-0087] That language is broad by design. It supports a chapter mechanism, not a detailed engineering verdict on one cooling architecture. pretend to settle the relative merits of liquid loops, air systems, immersion approaches, water strategies, or waste-heat reuse without a later technical source pack. What it can say now is that facility design became inseparable from model scaling.
...
The evidence pack does not support a single utilization number, and invent one. What it can do is make the categories legible. Interactive inference values responsiveness. Training and some synthetic-data jobs can be planned. Batch inference can sometimes tolerate delay. Internal enterprise workflows may be schedulable if the user is not waiting at the cursor. Coding agents sit in between: a developer may tolerate a longer background repair loop, but the product still has to feel alive when the agent is planning, explaining, or asking for approval. The infrastructure chapter should teach that "AI load" is not one load. It is a portfolio of workloads with different time sensitivity.

That portfolio matters for grid flexibility. DOE-SEAB's discussion of temporal/spatial compute flexibility, backup-power strategy, grid services, and model tariffs is important precisely because not every AI job has the same urgency. [S-0087] A future operator might move some compute to a different region, schedule it around lower-stress hours, or offer grid services under carefully designed rules. But the evidence row is a set of possible responses, not proof that the whole industry already runs as a dispatchable grid resource. the conditional mood. It can say that flexibility became a strategic question. It cannot say that flexibility solved the problem.
...
And it changed how the rest of the book reads. When the next chapter returns to chips, labs, agents, or model releases, this infrastructure shadow with them. A faster model is not only a research result. A cheaper answer is not only a pricing decision. A longer context window is not only a product setting. Each one implies a chain of physical accommodations somewhere below the interface. The point is not to make every LLM story into an electricity story. It is to make the invisible floor visible enough that the race can no longer float above it.

The interconnection queue became a useful analytical tool because it made the gap between announced capacity and deliverable power visible in a single number. A datacenter campus could be announced with a target capacity in megawatts. The grid operators responsible for delivering that power could have queues measured in years. The gap between announcement and delivery was not primarily a technology problem. It was a permitting, transmission, and coordination problem, and it meant that the speed of AI infrastructure deployment was governed as much by regulatory timelines and local grid conditions as by the speed of chip fabrication or construction.

## The Hormuz Shock and the Fragile Supply Chain
...
## Stargate: Infrastructure at the Half-Trillion Scale
...
## The Constraint Cascade: From GPUs to Gas to Memory to Oil
...
## The Inference Engine War: vLLM, SGLang, and the GPU
...
## LiveCodeBench and the Moving Target
...
Claude Code was not the only coding agent, and pretend otherwise. OpenAI's 2025 Codex agent, GitHub Copilot's evolution, Cursor-style editor agents, Devin-like systems, open-source terminal agents, and model-specific coding tools all belong in the landscape. [S-0054] But Claude Code is a clean case study because it concentrates the transition in one place: a frontier model, a terminal, a repository, permissions, context management, tests, and a user deciding how much agency to grant.
...
The relationship between Claude Code and the broader agent ecosystem also mattered in a specific competitive sense. While OpenAI built Codex into GitHub Copilot and Google integrated Gemini into its developer tools, Anthropic positioned Claude Code as a terminal-first tool that assumed the developer was already comfortable with the command line, Git, tests, and configuration files. This was not a product for everyone. It was a product for developers who were already operating at the repository level, and it made the assistant feel like an extension of the development environment rather than a separate service plugged into it. That positioning mattered because it defined a different kind of adoption curve: not mass-market convenience, but professional tool integration.
...
The reasoning era also changed the unit of price. Before reasoning models, a user typically paid per input token and per output token, with output costing more. Reasoning models added a third dimension: hidden computational tokens that the user did not see but still paid for. These tokens were consumed during the thinking process-chain-of-thought, search, verification, self-consistency-and then discarded before the final answer was shown. The user received a polished answer. The provider incurred the full cost of the reasoning trajectory. This created a pricing puzzle that the market had not fully solved by the cutoff. How do you charge for invisible work that makes the visible answer better, when the user cannot verify how much invisible work was actually done?

## Gemini 2.5 and the Reasoning Convergence
...
The competitive landscape hardened the economic puzzle. By early 2025, consumers could choose between ChatGPT Plus, Claude Pro, Gemini Advanced, and Perplexity Pro at roughly similar price points. Each subscription bundled different model families, context lengths, tool access, and feature velocity. For the user, $20 bought a very different object depending on which vendor issued the access. For the vendor, the same $20 represented a different gamble about inference costs, model routing, feature development, and user retention. OpenAI's Plus product faced the additional pressure of subsidizing free-tier users whose usage was effectively a marketing and training-data acquisition cost. Anthropic's Claude Pro competed on safety positioning and model quality rather than price. Google's Gemini Advanced was partly a bundle to keep users inside the Google ecosystem---a strategy that could absorb losses on the model itself if it preserved Search, Workspace, and cloud gravity.

This multi-vendor subscription landscape made the LLM market look less like a software industry and more like streaming media. Multiple providers competed on catalog depth (which models, which tools), user experience (speed, reliability, interface quality), and ecosystem lock-in. But unlike streaming, where content costs were amortized across subscribers, LLM inference costs scaled with usage. A power user asking a frontier reasoning model to solve a math problem might consume 50,000 tokens of thinking and response. A casual user asking for a recipe consumed a hundred tokens. The subscription price had to average these two behaviors across millions of users, and the averages could shift suddenly as model capabilities grew and user behavior adapted.
...
The economics story is therefore inseparable from the trust story in the next chapter. A model that is cheap but wrong costs more than a model that is expensive but reliable. A subscription that seems affordable can hide the cost of fact-checking, retrying, auditing, and fixing what the model produced. A token price is not the denominator. The real bill arrives when the answer generates work downstream.
